...
// 정렬 대상 컬럼 타입 (백엔드 _GROUP_SORT_FIELDS와 동일)
type SortField = "id" | "group_name" | "create_time" | "update_time";
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-gray-50 text-left text-muted-foreground">
              ...
              <SortableHeader field="group_name" label="그룹명" current={sortBy} order={sortOrder} onSort={handleSort} />
              {/* 권한 수는 배열 길이 계산값이므로 서버 정렬 불가 → 일반 헤더 */}
              ...
              data.items.map((group) => (
                <tr key={group.id} className="border-b last:border-0 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">
                    {group.create_time ? formatDate(group.create_time) : "-"}
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">
                    {group.update_time ? formatDate(group.update_time) : "-"}
                  </td>
