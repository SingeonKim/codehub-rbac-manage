...
- 로그인 후 `GET /api/auth/accessible-permissions` 호출
- 응답의 permission_name 목록에 `"CodeHubRbacManage"`가 포함되어야 서비스 이용 가능
- **권한 없는 경우**: 접근 불가 안내 페이지로 리다이렉트
...
| U-02 | 유저 검색 | user_id 완전 일치, full_name 완전 일치, department_name 부분 일치 (FastAPI 인메모리 필터링) |
...
├──────────────────────────────────────────────────┤
│  ID | 사용자ID | 이름 | 사번 | 부서 | 그룹 | 액션     │
│  ─────────────────────────────────────────────── │
│  1  | hong.gd | 홍길동 | 1234 | 개발팀 | Admin | ✏️🗑️ │
│  2  | kim.cs  | 김철수 | 1235 | 기획팀 | User  | ✏️🗑️ │
│  ...                                             │
├──────────────────────────────────────────────────┤
│  < 1 2 3 ... >                                   │  ← 페이지네이션
└──────────────────────────────────────────────────┘
